...
const OfficeModalForm = ({
    ...
    const [tellMissing, setTellMissing] = useState("");
    const [addressMissing, setAddressMissing] = useState("");
    const [occupantsMissing, setOccupantsMissing] = useState("");
    ...
    if (addressMissing && address) {
        setAddressMissing("");
    }
    if (occupantsMissing && occupants) {
        setOccupantsMissing("");
    }
    ...
    const submitHelper = () => {
        if (action === "Delete") {
            ...
            if (name && email && tell && address && occupants) {
                let office_info = {
                    name: name,
                    email: email,
                    tell_no: tell,
                ...
                if (!tell) {
                    setTellMissing("missing");
                }
                if (!address) {
                    setAddressMissing("missing");
                }
                if (!occupants) {
                    setOccupantsMissing("missing");
                }
            }
        }
    };
            ...
            if (action === "Delete") {
                return (
                    <Fragment>
                        ...
                        <h2 className='form-h2'>{currentOffice.name}</h2>
                        <button
